...
//	public static final InputStream INPUT_Stream = null;
    ...
    // Le enviamos al Handler el nuevo estado actual para que se actualize en la Actividad
        mHandler.obtainMessage(AppYateActivity.Mensaje_Estado_Cambiado, estado, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(AppYateActivity.Mensaje_Nombre_Dispositivo);
        Bundle bundle = new Bundle();
        bundle.putString(AppYateActivity.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(AppYateActivity.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(AppYateActivity.TOAST, "Error de conexión");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(AppYateActivity.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(AppYateActivity.TOAST, "Se perdio conexion");
        msg.setData(bundle);                       
        mHandler.sendMessage(msg);             
        msg = mHandler.obtainMessage(AppYateActivity.MESSAGE_Desconectado);                   
        mHandler.sendMessage(msg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(AppYateActivity.Mensaje_Escrito, -1, -1, buffer).sendToTarget();
        ...
        public  void read() {
            ...
            while (true) { //Mantiene escuchando el InputStream mientras este conectado
    				  ...
    				  String readMessageX = new String(readBufX, 0, bytes);//Se envia el readNessagexxx en lugar del buffer pues ya se PARSEO               
    				      mHandler.obtainMessage(AppYateActivity.Mensaje_Leido, bytes, -1, buffer).sendToTarget(); //readMessageX por buffer              
    				    cadena=readMessageX;
